...
namespace Flow
{
    class FilePanel
    {
        ...
        public void Show()
        {
            ...
            PsCon.PrintString(caption.ToString(), this.left + this.width / 2 - caption.ToString().Length / 2, this.top, ConsoleColor.White, ConsoleColor.Black);
        ...
        private void PrintObject(int index)
        {
            ...
            {
                Console.Write($"{0}{((DirectoryInfo)fsObjects[index]).CreationTime}\t{((DirectoryInfo)fsObjects[index]).Extension}");
            }
            else
            {
